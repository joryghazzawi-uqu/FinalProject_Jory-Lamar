...
public class SmartTaskManagerFrame extends JFrame {
    ...
    private void addTask() {
        ...
        try {
            ...
            Mono.fromRunnable(() -> taskManager.addTask(task))
                    ...
                    .doOnError(error ->
                            SwingUtilities.invokeLater(() ->
                                    showError("Add task failed", error)
                            )
                    )
                    .subscribe();
    ...
    private void updateWeatherForSelectedTask() {
        ...
        taskManager.fetchWeather(location)
                ...
                .doOnNext(forecast ->
                        SwingUtilities.invokeLater(() ->
                                updateWeatherStatus(taskId, forecast)
                        )
                )
                .doOnError(error ->
                        SwingUtilities.invokeLater(() ->
                                showError("Weather check failed", error)
                        )
                )
                .subscribe();
    }
     ...
     */
    private void updateWeatherStatus(String taskId, WeatherForecast forecast) {
        String status;

        if (forecast.getPrecipitationProbability() > 0.6) {
            status = "RISKY - rain expected";
        } else if (forecast.getTemperatureCelsius() >= 35) {
            status = "RISKY - high temperature";
        } else {
            status = "SAFE";
        }
    ...
    private void suggestSchedule() {
        ...
        Mono.fromCallable(taskManager::getTasks)
                ...
                .doOnError(error ->
                        SwingUtilities.invokeLater(() ->
                                showError("Suggestion failed", error)
                        )
                )
                .subscribe();
    }
